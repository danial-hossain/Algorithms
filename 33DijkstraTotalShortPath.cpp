#include<bits/stdc++.h>
using namespace std;
void DijkStraTotalSP(vector<pair<int,int>>adj[],int V,vector<int>&visited,vector<int>&distance,vector<int>&path)
{
    int count=V;
    ...
    while(count>0)
    {
        ...
        for(int i=0;i<adj[node].size();i++)
        {
          ...
          else   if(distance[adj[node][i].first]==distance[node] +weight)
            {
