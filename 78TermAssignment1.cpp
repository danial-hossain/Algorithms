...
const int MAX_WEIGHT = 20;          // Maximum edge weight,Uddipok e deya
...
void generateSparseGraph(int V, int E, vector<vector<int>> &edges)
{
    ...
    //edge.clear() kore to pura edge ta faka korlam,ekon bellman ford er jonno edge set {u,v,w} akta kore push korbo
    //johon oi edlist list amr given edge number er soman na hocce totokkhon loop cholbe
    {
       ...
       //incase eitay negative weight bosatam,will occur negative weight cycle
...
void generateSparseMatrix(int V, int E, vector<vector<int>> &matrix)
{
    ...
    for (int i = 0; i < V; i++)
        ...
        //Floyed warshall e diagonal/korno borabor element gula 0,avoiding self loop
        //nije theke nije theke distan

    set<pair<int, int>> used;
    srand(time(0));
    while ((int)used.size() < E)
        //for the sparse graph,edge hold the edge list,so we can use edge.size() <given edge number
        //for the matrix the matrix.size() will be the the number of row /vertices
        //so for the loop we haave to use the set.size() to stop the iteration
    {
        int u = rand() % V;
        int v = rand() % V;
        if (u == v) continue;
        if (used.count({u, v})) continue;
        //set list e u,v age thekle setake ar add krbe na,
        int w = MIN_WEIGHT + rand() % (MAX_WEIGHT - MIN_WEIGHT + 1);
        matrix[u][v] = w;
        used.insert({u, v});
    }
